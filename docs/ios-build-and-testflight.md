...
| 번들 ID | `ios/Runner.xcodeproj/project.pbxproj` 의 `PRODUCT_BUNDLE_IDENTIFIER` (예: `com.kuniverse.kUniverse`). 앱마다 고유. |
...
`CFBundleDisplayName` 이 홈 화면 이름이다(예: Korean-Universe).
...
python -X utf8 tools/ios/make_ios_cert.py --key-id XXXXXXXXXX --issuer-id <UUID> --p8 AuthKey_XXXXXXXXXX.p8 \
    --bundle-id com.kuniverse.kUniverse --app-name "Korean-Universe" --out ./ios_secrets
```
